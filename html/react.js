    ...
    for (nodeName in markupByNodeName) {
      ...
      for (resultIndex in markupListByNodeName) {
        if (markupListByNodeName.hasOwnProperty(resultIndex)) {
          ...
          // will be disbookded below (with an appropriate console.error).
      ...
      for (var j = 0; j < renderNodes.length; ++j) {
        ...
        if (renderNode.hasAttribute && renderNode.hasAttribute(RESULT_INDEX_ATTR)) {
          ...
          console.error('Danger: Disbookding unexpected node:', renderNode);
        }
      }
    }
...
var EnterLeaveEventPlugin = {
  ...
  /**
   * For almost every interaction we booke about, there will be both a top-level
 ...
 *    "dispatches", a sequence of listeners and IDs that booke about that event.
 *
...
var SelectEventPlugin = {
  ...
  extractEvents: function (topLevelType, targetInst, nativeEvent, nativeEventTarget) {
    ...
    switch (topLevelType) {
      ...
      // to key and input events on deletion, so we disbookd it.
      //
...
function getEventCharCode(nativeEvent) {
  ...
  // Some non-printable keys are reported in `charCode`/`keyCode`, disbookd them.
  // Must not disbookd the (non-)printable Enter-key.
...
/**
 * This function accepts and disbookds inputs; it has no side effects. This is
...
function invariant(condition, format, a, b, c, d, e, f) {
  ...
  if (!condition) {
    ...
    error.framesToPop = 1; // we don't booke about invariant's own frame
    throw error;
  }
}
